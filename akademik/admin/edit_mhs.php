...
$mhs = query("SELECT * FROM mahasiswa WHERE nim = '$nim1'")[0];
...
if(isset($_POST["submit"])) {
  ...
  $query = "UPDATE mahasiswa SET
            nim = '$nim', nama_mhs = '$nama', tgl_lahir = '$tgl_lahir', alamat = '$alamat', jenis_kelamin = '$jenis_kelamin', image = '$gambar' WHERE nim = '$nim1'";

  mysqli_query($conn, $query);

  if(mysqli_affected_rows($conn) > 0) {
    echo "<script>alert('Data Mahasiswa Berhasil Diubah!'); document.location.href = 'mahasiswa.php';</script>";
  } else {
    ...
    <nav>
      ...
      <section id="breadcrumb">
        <div class="container">
          ...
          <p class="breadcrumb-text"><a href="home.php">Home</a> &rsaquo; <a href="mahasiswa.php">Database Mahasiswa</a> &rsaquo; Edit Data Mahasiswa</p>
          </div>
        </div>
      </section>
...
<div class="content wrap">
    ...
    <!-- tambah data -->
<div class="text-left form mb-4">
      <h1 class="text-center text-gradient"> <i class="fa fa-user-graduate"> </i></h1>
      <h4 class="text-left"> Ubah Data Mahasiswa </h4> <hr> <br>

      <form action="" method="post" enctype="multipart/form-data">
        <input type="hidden" name="gambar_lama" value="<?= $mhs["image"]; ?>">
        <h6>1. Masukkan NIM Mahasiswa :</h6>
          <div class="form-floating mb-3">
          <input type="text" name="nim" id="nim" class="form-control" placeholder="Masukkan NIM Mahasiswa" value="<?= $mhs["nim"]; ?>" required>
          <label for="nim">NIM</label>
        </div>
        <br>
        <h6>2. Masukkan Nama Mahasiswa :</h6>
        <div class="form-floating mb-3">
            <input type="text" name="nama" id="nama" class="form-control" placeholder="Masukkan Nama Mahasiswa" value="<?= $mhs["nama_mhs"]; ?>" required>
            <label for="nama">Nama Mahasiswa</label>
        </div>
        <br>
        <h6>3. Masukkan Tanggal Lahir Mahasiswa :</h6>
        <div class="form-floating mb-3">
          <input type="date" name="tgl_lahir" id="tgl_lahir" class="form-control" placeholder="Masukkan Tanggal Lahir Mahasiswa" value="<?= $mhs["tgl_lahir"]; ?>" required>
          <label for="alamat">Alamat</label>
        </div>
        <br>
        <h6>4. Masukkan Alamat Mahasiswa :</h6>
        <div class="form-floating mb-3">
          <input type="text" name="alamat" id="alamat" class="form-control" placeholder="Masukkan Alamat Mahasiswa" value="<?= $mhs["alamat"]; ?>" required>
          <label for="alamat">Alamat</label>
        </div>
        <br>
        <h6>5. Pilih Jenis Kelamin Mahasiswa :</h6>
          <div class="form-floating mb-3">
          <select class="form-select" name="jenis_kelamin" id="jenis_kelamin" value="<?= $mhs["jenis_kelamin"]; ?>" required>
        ...
        <br>
          <div><img src="../images/<?= $mhs["image"] ?>"></div>
        <br>
        ...
        <button type="submit" name="submit" class="btn btn-primary btn-lg"><i class="fa fa-edit mr-2 icon"></i>  Ubah Data</button>
        </div>
      </form>
    </div>
